...
async function setUrl(prev: string): Promise<string> {
  let prevData = await sqlQuery(
    `SELECT * FROM url_set WHERE prev_url='${prev}'`
  );
  // 处理以前有过该链接的情况
  if (prevData.length === 1) return prevData[0].short_url;
    ...
    do {
      ...
      await sqlQuery(`SELECT * FROM url_set WHERE short_url='${hashStr}'`)
        .length
    );
...
async function getUrl(str: string): Promise<string> {
  let queryRes = await sqlQuery(
    `SELECT prev_url FROM url_set WHERE short_url='${str}'`
  );
...
export { getUrl, setUrl, setLog, connection };
